...
const ROLES = [
    { value: 'STATION_OPERATOR', label: 'Χειριστής Πρατηρίου' },
    { value: 'COMPANY_ADMIN', label: 'Διαχειριστής Εταιρείας' },
    { value: 'COMPANY_OPERATOR', label: 'Χειριστής Εταιρείας' },
    { value: 'CUSTOMS_REVIEWER', label: 'Ελεγκτής Τελωνείου' },
    { value: 'CUSTOMS_SUPERVISOR', label: 'Επόπτης Τελωνείου' },
    { value: 'CUSTOMS_DIRECTOR', label: 'Διευθυντής Τελωνείου' },
    { value: 'SYSTEM_ADMIN', label: 'Διαχειριστής Συστήματος' },
];
...
export default function UsersPage() {
    ...
    const resetForm = () => {
        setFormData({ email: '', password: '', role: 'STATION_OPERATOR', companyId: '', stationId: '' });
    };
                            ...
                            {filteredUsers.length === 0 ? (
                                ...
                                filteredUsers.map((u) => (
                                    <tr key={u.id} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-6 py-4 text-sm text-gray-500">{u.company?.name || '-'}</td>
                                        <td className="px-6 py-4 text-sm text-gray-500">{u.station?.name || '-'}</td>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                    <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
                        <div className="flex items-center justify-between mb-4">
                            <h2 className="text-xl font-bold">{editingUser ? 'Επεξεργασία Χρήστη' : 'Νέος Χρήστης'}</h2>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">
                                    Κωδικός {editingUser && '(αφήστε κενό για να μην αλλάξει)'}
                                </label>
                                <input
                                    type="password"
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    onChange={(e) => setFormData({ ...formData, companyId: e.target.value, stationId: '' })}
                                    className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                                >
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">Πρατήριο</label>
                                <select
                                ...
                                >
                                    ...
                                    {stations
                                        .filter((s) => s.companyId === formData.companyId)
                                        .map((s) => (
