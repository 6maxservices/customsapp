...
async function main() {
    ...
    const users = [
        { email: 'admin@alpha.gr', role: client_1.UserRole.COMPANY_ADMIN, companyId: alpha.id },
        { email: 'user@alpha.gr', role: client_1.UserRole.COMPANY_OPERATOR, companyId: alpha.id },
        { email: 'admin@beta.gr', role: client_1.UserRole.COMPANY_ADMIN, companyId: beta.id },
        { email: 'user@beta.gr', role: client_1.UserRole.COMPANY_OPERATOR, companyId: beta.id },
        { email: 'reviewer@customs.gov.gr', role: client_1.UserRole.CUSTOMS_REVIEWER, companyId: null },
        { email: 'admin@system.gov.gr', role: client_1.UserRole.SYSTEM_ADMIN, companyId: null },
    ];
    ...
    const obligationsData = [
        { code: 'OBL-001', title: 'ΤΗΡΗΣΗ ΣΤΟΙΧΕΙΩΝ ΦΟΡΟΛΟΓΙΚΟΥ ΜΗΤΡΩΟΥ', fieldType: client_1.ObligationFieldType.BOOLEAN, criticality: client_1.ObligationCriticality.CRITICAL },
        { code: 'OBL-002', title: 'ΑΔΕΙΑ ΛΕΙΤΟΥΡΓΙΑΣ ΠΡΑΤΗΡΙΟΥ', fieldType: client_1.ObligationFieldType.DATE, criticality: client_1.ObligationCriticality.CRITICAL }, // Needs Valid Date
        { code: 'OBL-003', title: 'ΒΕΒΑΙΩΣΗ ΟΓΚΟΜΕΤΡΗΣΗΣ ΔΕΞΑΜΕΝΩΝ', fieldType: client_1.ObligationFieldType.DATE, criticality: client_1.ObligationCriticality.MAJOR },
        { code: 'OBL-004', title: 'ΕΓΚΡΙΣΕΙΣ ΤΥΠΟΥ ΑΝΤΛΙΩΝ', fieldType: client_1.ObligationFieldType.BOOLEAN, criticality: client_1.ObligationCriticality.MAJOR },
        { code: 'OBL-005', title: 'ΥΠΕΥΘΥΝΗ ΔΗΛΩΣΗ ΣΥΝΕΡΓΕΙΟΥ', fieldType: client_1.ObligationFieldType.BOOLEAN, criticality: client_1.ObligationCriticality.MAJOR },
        { code: 'OBL-006', title: 'ΥΠΕΥΘΥΝΗ ΔΗΛΩΣΗ ΕΓΚΑΤΑΣΤΑΤΗ', fieldType: client_1.ObligationFieldType.BOOLEAN, criticality: client_1.ObligationCriticality.CRITICAL },
        { code: 'OBL-007', title: 'ΕΞΕΤΑΣΗ ΔΕΙΓΜΑΤΩΝ ΚΑΥΣΙΜΩΝ', fieldType: client_1.ObligationFieldType.BOOLEAN, criticality: client_1.ObligationCriticality.MINOR },
    ];
    const obligations = [];
    for (const o of obligationsData) {
        const ob = await prisma.obligation.upsert({
            ...
            update: {},
            create: { ...o, description: o.title, frequency: client_1.ObligationFrequency.PER_10_DAY, triggerAction: 'none', catalogVersionId: catalogVersion.id }
        });
        obligations.push(ob);
    }
    ...
    for (const station of createdStations) {
        ...
        for (const period of dbPeriods) {
            ...
            if (isProblematic) {
                ...
                // Good station
                if (rand < 0.02)
                    issueType = 'MISSING'; // Very rare miss
                else if (rand < 0.05)
                    issueType = 'OPEN_TASK'; // Rare task
            }
            // If Current Active Period (2026), nearly everyone hasn't submitted yet except maybe 1 or 2
            // @ts-ignore
            if (period.year === 2026) {
                if (Math.random() > 0.1)
                    continue; // 90% haven't submitted yet for current period
                status = 'DRAFT'; // Or submitted
            }
            if (issueType === 'MISSING')
                continue; // No submission created
            // Create Submission
            ...
            for (const obl of obligations) {
                ...
                else if (obl.fieldType === client_1.ObligationFieldType.DATE) {
